...
{
	HCRYPTHASH arg = ctx->hHash;
	CryptCreateHash(hCryptProv, CALG_MD5, 0, 0, &arg);
}
...
char* CalcChecksum(const char* path, char* md5str)
{
	...
	while(!feof(fInput))
	{
		size_t nCount = fread(bBuffer, sizeof(unsigned char), 4096, fInput);
		MD5Update(&md5Hash, bBuffer, (unsigned int)nCount);
	}
