...
Generate2p2::Generate2p2()
{
	IDs = new __int64[612978];
	HR = new int[32487834];
...
__int64 Generate2p2::MakeID(__int64 IDin, int newcard)  // adding a new card to this ID
{
	__int64 ID = 0;
	int suitcount[4 + 1];
	int rankcount[13 + 1];
...
int Generate2p2::SaveID(__int64 ID) 
{
	...
	if (ID >= maxID) 
	{ 
		// greater than create new else it was the last one!
		if (ID > maxID) 
		{  
			// add the new ID
			IDs[numIDs++] = ID;  
	...
	// do the insert into the hole created
	IDs[high] = ID;   
	numIDs++;        
	return high;
}

int Generate2p2::DoEval(__int64 IDin)
{
	...
	{	 
		...
		{  
			...
			// find out what suit (if any) was significant
		...
		{  
			...
			// my rank is top 4 bits 1-13 so convert
			rank = (workcard >> 4) - 1;
			// my suit is bottom 4 bits 1-4, order is different, but who cares?
			suit = workcard & 0xf;  
...
int Generate2p2::Generate_DAT()
{
	...
	memset(IDs, 0, sizeof(__int64)*612978);
	...
	{   
		...
		{	
			...
			if (numcards < 7) 
				holdid = SaveID(ID);   // and save it in the list if I am not on the 7th card
		}

		// just to show the progress -- this will count up to  612976
		printf("\rID - %d", IDnum);	  
	}
	...
	{  
		...
		// just to show the progress -- this will count up to  612976 same as above!
		printf("\rID - %d", IDnum);	  
	}
	...
	// now get current time for Testing!  
	timer = clock();   
	...
	printf("\nTotal Hands = %d\n", count);
	// calc clocks used from the High Precision clock
	__int64 clocksused = (__int64)endtimings.QuadPart - (__int64) timings.QuadPart;  
	...
	errno_t err_out;
	FILE * fout;
	err_out = fopen_s(&fout, "HandRanks.dat", "wb");
	...
	if (err_out != 0) 
	{
	 ...
	 // big write, but quick
	fwrite(HR, sizeof(int)*32487834, 1, fout); 
	fclose(fout);
